...
export const getProfiles = () => async (dispatch) => {
  try {
    dispatch({ type: CLEAR_PROFILE }); // Clear the current profile before loading all profiles, to avoid flashing the previous user's profile
...
// Get Github repos, github username as a parameter
...
export const deleteExperience = (id) => async (dispatch) => {
  try {
    const res = await axios.delete(
      `http://localhost:3001/api/profile/experience/${id}`
    );
