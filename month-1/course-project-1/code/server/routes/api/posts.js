...
router.put("/like/:id", auth, async (req, res) => {
  try {
    ...
    post.likes.unshift({ user: req.user.id }); //Add user to likes array, moves the user to the beginning of the array
    await post.save(); //Save the post
    res.json(post.likes); //Return the likes
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
  }
...
router.put("/unlike/:id", auth, async (req, res) => {
  try {
    ...
    const removeIndex = post.likes
      ...
      .indexOf(req.user.id); //Find the index of the like to remove, by user ID
    post.likes.splice(removeIndex, 1); //Remove like
    await post.save(); //Save the post
    res.json(post.likes); //Return the likes
  } catch (err) {
    console.error(err.message);
...
router.delete("/comment/:id/:comment_id", auth, async (req, res) => {
  try {
    ...
    const removeIndex = post.comments
      ...
      .indexOf(req.user.id); //Find the index of the comment to remove, by user ID
